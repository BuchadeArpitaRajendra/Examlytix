(function () {
  ...
  document.body.addEventListener("click", function (e) {
    ...
    if (kind === "candidate") {
      openModal("Edit candidate", "Candidate #" + id, `/admin/api/candidates/${id}`,
        field("Name", "name", btn.dataset.name) +
        field("Email", "email", btn.dataset.email, "email") +
  ...
  document.body.addEventListener("click", function (e) {
    ...
    const id = btn.dataset.id;
    if (!confirm(`Delete this ${kind} record? This cannot be undone.`)) return;
  ...
  modalForm.addEventListener("submit", function (e) {
    ...
    formData.forEach((value, key) => {
      if (key === "password" && !value) return; // skip blank password
      payload[key] = value;
    });
    ...
    fetch(currentEndpoint, {
      ...
      .then(data => {
        if (data.success) {
          ...
          modalErrors.innerHTML = `<div class="alert alert-error">${(data.errors || ["Update failed."]).join("<br>")}</div>`;
        }
      })
      .catch(() => {
        modalErrors.innerHTML = `<div class="alert alert-error">Network error. Please try again.</div>`;
      });
  });
})();
